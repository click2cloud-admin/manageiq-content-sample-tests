...
def get_template(build, merged_options_hash, merged_tags_hash)
  ...
  if template_search_by_os && templates.blank?
    case merged_tags_hash[:environment]
      when 'dev';
        vmdb_table = :ManageIQ_Providers_Openstack_CloudManager_Template
      when 'prod';
        vmdb_table = :ManageIQ_Providers_Vmware_InfraManager_Template
      when 'test';
        vmdb_table = :ManageIQ_Providers_Amazon_CloudManager_Template
      when 'qa';
        vmdb_table = :ManageIQ_Providers_Redhat_InfraManager_Template
    end
    os_category = 'os'
    log(:info, "Searching for templates tagged with #{@rbac_array} and "\
...
def get_network(build, merged_options_hash, merged_tags_hash)
  ...
  case @template.vendor.downcase
    ...
    when 'amazon'
      ...
      # 2017-08-07 Jeff Warnica
      #
      # Here we demonstrate "cloud_network" placement, by the value of the Environment tag.
      # (An AWS VPC is a CloudForms cloud_network). Its as bad a place as any for a quick walk through
      # ruby.
      #
      # @template was set earlier, and is the source AMI
      #   ext_management_system is the top level handle of an AWS provider (user/pass/region)
      #   cloud_networks is an array of objects, all the cloud_networks (VPC) in the ext_management_system
      #   find{|cn| ... } loops on the cn local var, returning that cn when the criteria hits
      #
      # Available cloud_subnet's are attached to cloud_networks; find one of those too
      #
      # merged_options_hash is the big structure we send when we kick off the literal VM (er, instance) provsioning
      #
      # Note: that because reasons, we need to set in both xxx and xxx_id for cloud_network and cloud_subnet
      #

      env = merged_tags_hash[:environment].downcase rescue ''
      type = merged_tags_hash[:type].downcase rescue ''

      desired_cn = env
      desired_csn = "#{env}-private"
      #desired_sg = "#{env}-#{type}"
      desired_sg = "#{env}"

      log(:info, "Build: #{build} - searching for VPC [#{desired_cn}], subnet [#{desired_csn}], security group: [#{desired_sg}]")

      err = nil
      cloud_network = @template.ext_management_system.cloud_networks.find {|cn| cn.name.downcase == desired_cn}
      if cloud_network.nil?
        err = "Unable to locate suitable cloud_network: [#{desired_cn}]"
        log(:error, err)
        raise err
      end

      cloud_subnet = cloud_network.cloud_subnets.find {|csn| csn.name.downcase.downcase == desired_csn}
      security_group = @template.ext_management_system.security_groups.find {|sg| sg.name.downcase == desired_sg}


      if cloud_subnet.nil?
        err = "Unable to locate suitable cloud_subnet: [#{desired_csn}]"
      end
      if security_group.nil?
        err = "Unable to locate suitable security group: [#{desired_sg}]"
      end
      unless err.nil?
        log(:error, err)
        raise err
      ...
      merged_options_hash[:cloud_subnet] = cloud_subnet.id
      merged_options_hash[:cloud_subnet_id] = cloud_subnet.id
      ...
      key_pair = provider.key_pairs.detect {|kp| kp.name == 'ansible'}
      merged_options_hash[:guest_access_key_pair] = key_pair.id
      merged_options_hash[:guest_access_key_pair_name] = key_pair.name

      log(:info, "Build: #{build} key_pair_name: #{merged_options_hash[:guest_access_key_pair_name]}")
  ...
  case @template.vendor.downcase
    when 'openstack', 'amazon';
      ...
      case flavor_search_criteria.gsub(/^\d*_/, '').match(/\w*$/)[0]
        when 'xsmall';
          flavor_name, number_of_sockets, cores_per_socket, vm_memory = 'xsmall', 1, 1, 1024
        when 'small';
          flavor_name, number_of_sockets, cores_per_socket, vm_memory = 'small', 1, 1, 2048
        when 'medium';
          flavor_name, number_of_sockets, cores_per_socket, vm_memory = 'medium', 1, 2, 4096
        when 'large';
          flavor_name, number_of_sockets, cores_per_socket, vm_memory = 'large', 1, 4, 8192
        when 'xlarge';
          flavor_name, number_of_sockets, cores_per_socket, vm_memory = 'xlarge', 1, 8, 16384
        else
          # default to small
          flavor_name, number_of_sockets, cores_per_socket, vm_memory = 'small', 1, 1, 2048
      end
...
def process_builds(dialog_options_hash, dialog_tags_hash)
  ...
  builds.each do |build|
    ...
    buildRequestIds << request.id
    log(:info, "Build: #{build} - VM Provision request #{request.id} for " \
        "#{merged_options_hash[:vm_name]} successfully submitted", true)
  end

  # Jeff Warnica: CF service building generally doesn't keep track of the VM builds
  # So do that. As CSV because it only takes a scalar
  @task.set_option('build_request_ids', buildRequestIds.join(','))
end
...
def build_provision_request(build, merged_options_hash, merged_tags_hash)
  ...
  cur_vm = 1
  count = merged_options_hash[:count].to_i
  # while cur_vm <= count do
  log(:info, "Build: #{build} - Count: [#{cur_vm}] of [#{count}] - Building provision request with the following arguments: #{args.inspect}")
